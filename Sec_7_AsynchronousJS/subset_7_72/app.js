...
const api_url_put = `https://jsonplaceholder.typicode.com/users/${updated_id}`;

http.put(api_url_put, user).then(data => {
    displayOuput(data);
...
http.delete(api_url).then(data => {
    ...
    if(Object.keys(data).length === 0 && data.constructor === Object) displayOuput('Resource Removed');
